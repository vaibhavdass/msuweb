 ...
 */
abstract class Pwa_PaywithAmazon_Model_Api_Client_Abstract extends Pwa_PaywithAmazon_Model_Abstract {
    ...
    public function __construct() {
        ...
        $this->_config['UserAgent'] = 'Pwa Checkout by Amazon Magento Extension/' . (string) Mage::getConfig()->getNode('modules/Pwa_PaywithAmazon/version')
    ...
    protected function _handleError($response, $status) {
        ...
        Mage::helper('paywithamazon')->throwException($message, $status, array('area' => $this->_area, 'request_id' => $requestId));
    ...
    protected function _httpPost($params) {
        ...
        if ($socket = @fsockopen($scheme . $url['host'], $port, $errno, $errstr, 10)) {
            ...
            Mage::helper('paywithamazon')->throwException(
                Mage::helper('paywithamazon')->__('Unable to establish connection to host %s: %s', $url['host'], $errstr),
                $errno,
    ...
    protected function _pauseBeforeRetry($retries, $status) {
        if ($retries <= $this->_config['MaxErrorRetry']) {
            ...
            Mage::helper('paywithamazon')->throwException(
                Mage::helper('paywithamazon')->__('Maximum number of retry attempts to Amazon Payments API reached: %s', $retries),
                $status,
                array('area' => $this->_area)
            );
        }
    ...
    protected function _sign(array $params) {
        ...
        switch ($this->_config['SignatureMethod']) {
            ...
            case 'HmacSHA1':
                ...
                break;
            default:
                Mage::helper('paywithamazon')->throwException('Non-supported signing method specified', null, array('area' => $this->_area));
        }
    ...
    public function __call($function, $params) {
        ...
        if (class_exists($classes['response'], true) && isset($params[0]) && $params[0] instanceof $classes['request'] && method_exists($classes['request'], 'convertToQueryString')) {
            ...
            Mage::helper('paywithamazon')->throwException(Mage::helper('paywithamazon')->__('Invalid method %s::%s (%s)', get_class($this), $function, print_r($params, 1)), null, array('area' => $this->_area));
        }
    }
    ...
    public function processRequest($request) {
        if (method_exists($request, 'convertToQueryString')) {
            ...
            Mage::helper('paywithamazon')->throwException(Mage::helper('paywithamazon')->__('Request %s cannot be converted to query string', get_class($this)), null, array('area' => $this->_area));
        }
    }

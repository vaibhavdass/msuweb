...
class IllApps_Shipsync_Model_Shipping_Carrier_Fedex extends Mage_Usa_Model_Shipping_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    protected function _initWebServices($wsdlPath)
    {
        ...
        $shipping_country = Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getCountry();
        ...
        } else if($shipping_country == 'IN') {
            $auth = 'RgHqVxZx0tqBg74O';
            $pwd = 'WlSPRr8M7J0XFTHDHQv5uuUCC';
            $acc_num = 719985106;
            $meter_num = 108854228;
            $this->setFedexKey($auth);
            $this->setFedexPassword($pwd);
            $this->setFedexAccount($acc_num);
            $this->setFedexMeter($meter_num);

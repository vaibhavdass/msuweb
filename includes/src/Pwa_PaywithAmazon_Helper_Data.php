 ...
 */
class Pwa_PaywithAmazon_Helper_Data extends Mage_Core_Helper_Abstract {
     ...
     * @param int $code
     * @throws Pwa_PaywithAmazon_Exception
     */


    public function throwException($message, $code = 0, $params = array()) {
        $exception = Mage::exception('Pwa_PaywithAmazon', $message, $code);
    ...
    public function getPaywithAmazonButton() {
        return Mage::getSingleton('core/layout')->createBlock('PaywithAmazon/link')->setTemplate('pwa/PaywithAmazon/link.phtml')->toHtml();
    }

    public function getPaywithAmazonBadge() {
        switch ($this->getConfigData('marketplace')) {
            case 'de_DE':
                return '<img src="' . Mage::getDesign()->getSkinUrl('pwa/images/amazon-payments-badge.de.gif') . '" alt="' . $this->__('Checkout by Amazon') . '" title="' . $this->__('Checkout by Amazon') . '" />';
            default:
                return '<img src="' . Mage::getDesign()->getSkinUrl('pwa/images/amazon-payments-badge.png') . '" alt="' . $this->__('Checkout by Amazon') . '" title="' . $this->__('Checkout by Amazon') . '" />';
        }
    }
    ...
    public function lockExceptionLogging() {
        return Pwa_PaywithAmazon_Model_Abstract::lockExceptionLogging();
    }

    public function unlockExceptionLogging() {
        return Pwa_PaywithAmazon_Model_Abstract::unlockExceptionLogging();
    }

    public function isExceptionLoggingLocked() {
        return Pwa_PaywithAmazon_Model_Abstract::isExceptionLoggingLocked();
    }
    public function getConfigData($key, $params = array()) {
        return Pwa_PaywithAmazon_Model_Abstract::getConfigData($key, $params);
    }
	...
	protected function _isIpAllowed() {
        $allowedIps = $this->getConfigData('allowed_ips');
    ...
    protected function _isActive() {
        return $this->getConfigData('active') && $this->_isIpAllowed();
    }
    
	public function getDecodeCart($cartXML)
	{
		return base64_decode($cartXML);
	}
	
	public function addPWAJs()
    {
        if($this->_isActive() && (Mage::getStoreConfig('paywithamazon/design/minicart') || Mage::app()->getRequest()->getModuleName() == 'checkout')){
            return '<script type="text/javascript" src="' . $this->getHeadJs() . '"></script>';
        }
        return '';
    }
    public function getEasyShipStatus()
    {      
        return Mage::getStoreConfig('paywithamazon/general/easy_ship_active');
    }
    public function getAwsActiveStatus()
	{   
		return Mage::getStoreConfig('paywithamazon/general/active');
	}
    public function getCouponCodeMaxLength()
    {   
        return defined('Mage_Checkout_Helper_Cart::COUPON_CODE_MAX_LENGTH') ? Mage_Checkout_Helper_Cart::COUPON_CODE_MAX_LENGTH : 255; 
            //Mage_Checkout_Helper_Cart::COUPON_CODE_MAX_LENGTH = 255; According to magento 1.9 and 1.8
    }
}

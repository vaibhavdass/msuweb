...
class Mirasvit_Advr_Model_Report_Sales extends Mirasvit_Advr_Model_Report_Abstract
{
    ...
    protected function _construct()
    {
        ...
        */
        $isNewCustomer = 'sales_order_email_table.customer_email IS NULL';
        ...
        $this->addColumn(
            ...
            array(
                'label' => 'Qty Ordered',
                'type' => 'number',
                'expression' => 'SUM(distinct sales_order_item_table.qty_ordered)',
                'table' => 'sales/order_item',
            )
        );
    ...
    public function joinNewWithReturning()
    {
        //$considerNotloggedCustomers = Mage::getSingleton('advr/config')->isConsiderNotloggedCustomers();
        //if ($considerNotloggedCustomers){
            $tableName = 'sales_order_email_table';
            ...
            array($tableName => new Zend_Db_Expr(
                '(SELECT DISTINCT s2.created_at, s2.customer_email'.
                ' FROM '.$this->getTable('sales/order').' s1 '.
                ' INNER JOIN '.
                '(SELECT created_at, customer_email '.
                ' FROM '.$this->getTable('sales/order').
                ' WHERE created_at BETWEEN "'.$this->getFilterData()->getFrom().'" AND "'.$this->getFilterData()->getTo().'") s2 '.
                ' ON (s1.customer_email = s2.customer_email) AND (s1.created_at < s2.created_at)'.
                ')'
            )),
            $tableName.'.customer_email = sales_order_table.customer_email AND '.$tableName.'.created_at = sales_order_table.created_at',
            array('created_at', 'customer_email')
            );

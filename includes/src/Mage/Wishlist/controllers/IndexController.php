...
class Mage_Wishlist_IndexController extends Mage_Wishlist_Controller_Abstract
{
    ...
    protected function _addItemToWishList()
    {
        ...
        try {
            ...
            $quote1 = Mage::getSingleton('checkout/cart')->getQuote();
            $allitems = $quote1->getAllItems();
            $quote_val = $quote1->getId();
            $count = 0;
            Mage::log($pid,null,'wishlist.log');
            if(sizeof($allitems) > 0){
                foreach ($allitems as $item) {
                    Mage::log($item->getItemId(),null,'cart.log');
            ...
            if($count == 0) {
                $service_id = Mage::getModel('newaddaction/newaddaction')->getCollection()->addFieldToFilter('quote_id',$quote_val)->addFieldToFilter('product_id',$product->getId())->getFirstItem();
                Mage::getModel('newaddaction/newaddaction')->load($service_id->getServiceId())->delete();
                $measurement_id = Mage::getModel('measurementremember/measurementremember')->getCollection()->addFieldToFilter('quote_id',$quote_val)->addFieldToFilter('product_id',$product->getId())->getFirstItem();
                Mage::getModel('measurementremember/flatrates')->load($measurement_id->getFlatrateId())->delete();
                Mage::getModel('measurementremember/measurementremember')->load($measurement_id->getMeasurementrememberId())->delete();
            }

...
class Apptha_Onestepcheckout_Adminhtml_OnestepcheckoutController extends Mage_Adminhtml_Controller_action
{
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($onepagecheckoutIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($onepagecheckoutIds)) {
            ...
            try {
                foreach ($onepagecheckoutIds as $onepagecheckoutId) {
                    $onepagecheckout = Mage::getSingleton('Onestepcheckout/Onestepcheckout')
                        ->load($onepagecheckoutId)
                        ->setStatus($this->getRequest()->getParam('status'))
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($onepagecheckoutIds))
                );
            } catch (Exception $e) {

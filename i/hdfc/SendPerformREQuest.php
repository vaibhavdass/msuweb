...
$ReqResponseUrl="responseURL=http://www.mysoresareeudyog.com/hdfc/GetHandleRESponse.php";
...
$ReqErrorUrl="errorURL=http://www.mysoresareeudyog.com/index.php/checkout/onepage/failure/";
...
$response = $data1;
            ...
            try
			{
				// Mage::log($response, null, 'naresh.log');
				$index=strpos($response,"!-");
				// Mage::log($index, null, 'naresh1.log');
				$ErrorCheck=substr($response, 1, $index-1);//This line will find Error Keyword in response
				// Mage::log($ErrorCheck, null, 'naresh2.log');
				if($ErrorCheck == 'ERROR')//This block will check for Error in response
				{
					// here redirecting the error page 
					$failedurl='http://demo.mysoresareeudyog.com/index.php/checkout/onepage/failure?ResTrackId='.$TranTrackid.'&ResAmount='.$TranAmount.'&ResError='.$response;
			...
			{
				Mage::log($e->getMessage(), null, 'naresh87.log');

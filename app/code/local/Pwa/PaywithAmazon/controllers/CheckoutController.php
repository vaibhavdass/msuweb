 ...
 */
class Pwa_PaywithAmazon_CheckoutController extends Mage_Core_Controller_Front_Action {
    ...
    protected function _getPaymentMethod() {
        if (Mage::helper('paywithamazon')->getConfigData('mode') == 'sandbox')
            return array('method' => 'paywithamazon_sandbox');

        return array('method' => 'paywithamazon');
    }
    ...
    protected function _expireAjax() {
        if (!$this->_getOnepage()->getQuote()->hasItems()
            ...
            || ($this->_getOnepage()->getCheckoutMehod()
                && !in_array($this->_getOnepage()->getCheckoutMehod(), array(Pwa_PaywithAmazon_Model_Abstract::CHECKOUT_METHOD_AMAZON, Mage_Checkout_Model_Type_Onepage::METHOD_CUSTOMER)))) {
    ...
    protected function _getAmazonCheckoutApi() {
        if (null === $this->_amazonCheckoutApi) {
            $this->_amazonCheckoutApi = Mage::getModel('paywithamazon/api_checkout');
        }
        return $this->_amazonCheckoutApi;
    }

    protected function _getAmazonPrice($amount) {
        return Mage::getModel('paywithamazon/api_model_checkout_price', array(
            'Amount' => Mage::helper('paywithamazon')->sanitizePrice($amount)
        ));
    }
    ...
    protected function _getReviewHtml() {
        $layout = $this->getLayout();
        $update = $layout->getUpdate();
        $update->load('paywithamazon_checkout_review');
        $layout->generateXml();
        $layout->generateBlocks();
        $output = $layout->getOutput();
        return $output;
    ...
    protected function _getOnepage() {
        return Mage::getSingleton('paywithamazon/checkout');
    }
    ...
    protected function _validateCountry($countryCode) {
        ...
        if (Mage::getStoreConfigFlag(Pwa_PaywithAmazon_Model_Abstract::XML_PATH_COUNTRY_ALLOW)) {
            $availableCountries = explode(',', strtoupper(Mage::getStoreConfig(Pwa_PaywithAmazon_Model_Abstract::XML_PATH_SPECIFIC_COUNTRY)));
        } else {
    ...
    public function indexAction() {
        ...
        $this->_getOnepage()->saveCheckoutMethod(Pwa_PaywithAmazon_Model_Abstract::CHECKOUT_METHOD_AMAZON);
        ...
        try {
            ...
            if (isset($addressList[0])) {
                ...
                if (count($customerName) > 1) {
                    ...
                    $firstname = Mage::helper('paywithamazon')->__('n/a');
                    $lastname = reset($customerName);
                }

                $regionId = Mage::helper('paywithamazon')->getRegionId($addressModel->getStateOrProvinceCode(), $addressModel->getCountryCode());
                ...
                if (Mage::getSingleton('eav/config')->getAttribute('customer', 'prefix')->getIsRequired()) {
                    $prefix = Mage::helper('paywithamazon')->getConfigData('default_customer_prefix');
                }
                ...
                $address = array(
                    ...
                    'lastname' => $lastname,
                    'company' => '',
                    'email' => Mage::helper('paywithamazon')->getConfigData('email_placeholder'),
                    'street' => $street,
                ...
                if ($this->_validateCountry($addressModel->getCountryCode())) {
                    ...
                    $result['error'] = 1;
                    $result['message'] = Mage::helper('paywithamazon')->__('Sorry. We do not deliver to this country. Please select another shipping address.');
                }
    ...
    public function saveOrderAction() {
        ...
        $result = array();
        try {
            Mage::helper('paywithamazon')->lockExceptionLogging();
            ...
            if (!$order->getId()) Mage::helper('paywithamazon')->throwException('Cannot load order with given ID', 0, array('area' => 'Inline Checkout'));

            $purchaseContractId = $this->getRequest()->getParam('purchaseContractId', null);
            $itemList = Mage::getModel('paywithamazon/api_model_checkout_itemList');
            ...
            foreach ($order->getItemsCollection() as $item) {
                ...
                $amazonItem = Mage::getModel('paywithamazon/api_model_checkout_purchaseItem', array(
                    'MerchantItemId' => $item->getId(),
                    'SKU' => Mage::helper('paywithamazon')->sanitizeSku($item->getSku()),
                    'Title' => $item->getName(),
                    'UnitPrice' => array('Amount' => Mage::helper('paywithamazon')->sanitizePrice($itemUnitPrice)),
            ...
            $amazonCharges = Mage::getModel('paywithamazon/api_model_checkout_charges');
            if (abs($order->getBaseDiscountAmount()) > 0) {
                $amazonPromotion = Mage::getModel('paywithamazon/api_model_checkout_promotion', array(
                    'PromotionId' => (trim($order->getCouponCode()) ? trim($order->getCouponCode()) : Mage::helper('paywithamazon')->__('Discount')),
                    'Description' => (trim($order->getDiscountDescription()) ? trim($order->getDiscountDescription()) : Mage::helper('paywithamazon')->__('Discount')),
                    'Discount' => array('Amount' => abs($order->getBaseDiscountAmount()))
                ));
                $promotionList = Mage::getModel('paywithamazon/api_model_checkout_promotionList');
            ...
            if (!$setItemsResult) Mage::helper('paywithamazon')->throwException('Error when performing SetItems call', 0, array('area' => 'Inline Checkout'));

            $setContractChargesResult = $this->_getAmazonCheckoutApi()->setContractCharges($purchaseContractId, $amazonCharges);
            if (!$setContractChargesResult) Mage::helper('paywithamazon')->throwException('Error when performing SetContractCharges call', 0, array('area' => 'Inline Checkout'));
            ...
            if (!is_null($amazonOrderIdList)) {
                ...
                Mage::helper('paywithamazon')->throwException('Error when performing CompletePurchaseContract call', 0, array('area' => 'Inline Checkout'));
            }
            ...
            Mage::helper('paywithamazon')->unlockExceptionLogging();
    ...
    public function successAction() {
        //echo "herer".$this->_getOnepage()->getCheckout()->getLastSuccessQuoteId()."in";

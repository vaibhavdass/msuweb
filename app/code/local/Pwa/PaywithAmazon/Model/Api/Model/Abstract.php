 ...
 */
abstract class Pwa_PaywithAmazon_Model_Api_Model_Abstract extends Pwa_PaywithAmazon_Model_Abstract {
    ...
    private function _isApiModelClass($type) {
        return preg_match('/^Pwa_PaywithAmazon_Model_Api_Model_/', $type);
    }

    private function _isApiObject($value) {
        if (gettype($value) == 'object') {
            return preg_match('/^Pwa_PaywithAmazon_Model_Api_Model_/', get_class($value));
        }
        return false;
    }
        ...
        if (!is_null($data)) {
            if ($this->_isHashTable($data)) {
                ...
                Mage::helper('paywithamazon')->throwException(
                    Mage::helper('paywithamazon')->__('Unable to construct Amazon API model: %s from provided data. Please be sure to pass associative array or DOMElement.', get_class($this)),
                    null,

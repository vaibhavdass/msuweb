...
abstract class Pwa_PaywithAmazon_Model_Abstract {

    // general settings
    const XML_PATH_ACTIVE                   = 'paywithamazon/general/active';
    const XML_PATH_IOPN_ACTIVE              = 'paywithamazon/general/iopn_active';
    const XML_PATH_MARKETPLACE              = 'paywithamazon/general/marketplace';
    const XML_PATH_EMAIL_PLACEHOLDER        = 'paywithamazon/general/email_placeholder';
    const XML_PATH_DEFAULT_PREFIX           = 'paywithamazon/general/default_prefix';
    const XML_PATH_WEIGHT_UNIT              = 'paywithamazon/general/weight_unit';   // TB added
    const XML_PATH_CUT_SKU                  = 'paywithamazon/general/cut_sku';
    const XML_PATH_SANDBOX_MODE             = 'paywithamazon/general/sandbox_mode';
    const XML_PATH_NEW_ORDER_STATUS         = 'paywithamazon/general/new_order_status';
    const XML_PATH_ORDER_CONFIRMATION       = 'paywithamazon/general/order_confirmation';

    // authorization data
    const XML_PATH_MERCHANT_ID              = 'paywithamazon/general/merchant_id';
    const XML_PATH_ACCESS_KEY_ID            = 'paywithamazon/general/access_key';
    const XML_PATH_SECRET_KEY               = 'paywithamazon/general/secret_key';

    // availability settings
    const XML_PATH_COUNTRY_ALLOW            = 'paywithamazon/general/sallowspecific';
    const XML_PATH_SPECIFIC_COUNTRY         = 'paywithamazon/general/specificcountry';

    // Amazon MWS settings
    const XML_PATH_MWS_ORDER_UPDATE         = 'paywithamazon/api/order_update';
    const XML_PATH_REPORT_SCHEDULE          = 'paywithamazon/api/report_schedule';
    const XML_PATH_MWS_USE_TOKEN            = 'paywithamazon/api/use_token';
    const XML_PATH_FEED_BATCHING            = 'paywithamazon/api/feed_batching';
    const XML_PATH_FEED_SCHEDULE            = 'paywithamazon/api/feed_schedule';
    const XML_PATH_LAST_FEED_SENDING        = 'paywithamazon/api/last_feed_sending';
    const XML_PATH_LAST_REPORT_PROCESSING   = 'paywithamazon/api/last_report_processing';
    const XML_PATH_LAST_ORDER_PROCESSING    = 'paywithamazon/api/last_order_processing';

    // debugging options
    const XML_PATH_ALLOWED_IPS              = 'paywithamazon/general/allow_ips';
    const XML_PATH_LOGGER_ENABLED           = 'paywithamazon/logger/active';
    const XML_PATH_LOGGER_CLEAN_AFTER       = 'paywithamazon/logger/clean_after_day';

    const GLOBAL_CONFIG_CBA_NODE            = 'global/pwa/paywithamazon';
    ...
    protected $_modulePrefix = 'paywithamazon';
    ...
    private static function _getApiEndpointUrl($api, $marketplace = 'en_US', $mode = 'live') {
        switch ($api) {
            ...
            default:
                Mage::helper('paywithamazon')->throwException('API name invalid or not specified');
        }
    }
    ...
    protected function _generateHmacSignature($data, $algorithm = self::HMAC_SHA1_ALGORITHM) {
        switch (strtolower($algorithm)) {
            ...
            case self::HMAC_SHA256_ALGORITHM:
                break;
            default :
                Mage::helper('paywithamazon')->throwException('Non-supported signing method specified');
        }
    ...
    protected function _getAmazonPaymentMethods() {
        return array('amazonpayments', 'paywithamazon', 'paywithamazon_sandbox','amazoncod');
    }
    ...
    public static function getConfigData($key, $params = array()) {
        switch ($key) {
            ...
            case 'iopn_schema':
                return Mage::getBaseDir() . DS . str_replace('<codePool>', (string)Mage::getConfig()->getNode('modules/Pwa_PaywithAmazon/codePool'), (string)Mage::getConfig()->getNode(self::GLOBAL_CONFIG_CBA_NODE . '/api/iopn/schema_path'));

            // returns IOPN url, if in sandbox non-secure URL can be used
            case 'iopn_url':
                switch (self::getConfigData('mode')) {
                    case 'sandbox':
                        return Mage::getUrl('paywithamazon/process/iopn', array('_nosid' => true));
                    default:
                        return Mage::getUrl('paywithamazon/process/iopn', array('_nosid' => true, '_forced_secure' => true));
                }
            ...
            case 'marketplace_id':
                switch (self::getConfigData('marketplace')) {
					...
					case 'hi_IN':
                        return (string)Mage::getConfig()->getNode(self::GLOBAL_CONFIG_CBA_NODE . '/api/marketplace/ids/hi_IN');
                    default:
                        Mage::helper('paywithamazon')->throwException('Unknown marketplace');
                }

...
class Naresh_Taxrate_Adminhtml_TaxrateController extends Mage_Adminhtml_Controller_action
{
	...
	public function saveAction() {
		if ($data = $this->getRequest()->getPost()) {
			...
			try {
				...
				// $query = 'UPDATE taxrate SET cat_id = "'.$cat_id .'" WHERE taxrate_id = '.$model->getId();
				// $write->query($query);
				// $val = Mage::getModel('taxrate/flatrates')->saveMultipleFlatrates($data, $model->getId(),$cat_id);
				// if ($val == -1) {
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				// Mage::getModel('taxrate/flatrates')->deleteVal($this->getRequest()->getParam('id'));
				$model = Mage::getModel('taxrate/taxrate');
        ...
        if(!is_array($taxrateIds)) {
            ...
            try {
                foreach ($taxrateIds as $taxrateId) {
                	// Mage::getModel('taxrate/flatrates')->deleteVal($taxrateId);

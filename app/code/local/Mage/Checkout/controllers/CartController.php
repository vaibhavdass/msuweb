        ...
        if ($id) {
            try {
                ...
                if(sizeof($allitems) > 0){
                    $quote_val = Mage::getSingleton('checkout/session')->getQuote()->getId();
                    foreach ($allitems as $item) {
                        if($item->getItemId() == $id) {
                            $service_id = Mage::getModel('newaddaction/newaddaction')->getCollection()->addFieldToFilter('quote_id',$quote_val)->addFieldToFilter('product_id',$item->getProductId())->getFirstItem();
                            Mage::getModel('newaddaction/newaddaction')->load($service_id->getServiceId())->delete();
                            $measurement_id = Mage::getModel('measurementremember/measurementremember')->getCollection()->addFieldToFilter('quote_id',$quote_val)->addFieldToFilter('product_id',$item->getProductId())->getFirstItem();

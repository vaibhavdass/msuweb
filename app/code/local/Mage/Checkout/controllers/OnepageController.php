...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function successAction()
    {
        ...
        mail('it_magento@mysoresareeudyog.com', 'Order Placed Status', $lastOrderId);
    ...
    public function failureAction()
    {
        ...
        mail('it_magento@mysoresareeudyog.com', 'Order Failure Status', $lastOrderId);
    ...
    public function saveOrderAction()
    {
        ...
        // Mage::log('Start',null,'checkout.log');
        // Mage::log('Order ID',null,'checkout.log');
        // Mage::log(Mage::getSingleton('checkout/session')->getLastOrderId(),null,'checkout.log');
        mail('it_magento@mysoresareeudyog.com', 'New Order Placed', Mage::getSingleton('checkout/session')->getLastRealOrderId());
        ...
        // Mage::log('All Available Stitching Services',null,'checkout.log');
        // Mage::log($options,null,'checkout.log');

        $base_currency = Mage::app()->getStore()->getBaseCurrencyCode();
        $current_currency =  Mage::app()->getStore()->getCurrentCurrencyCode();
        // Mage::log('Base Currency',null,'checkout.log');
        // Mage::log($base_currency,null,'checkout.log');
        // Mage::log('Current Currency',null,'checkout.log');
        // Mage::log($current_currency,null,'checkout.log');
        ...
        // Mage::log('DDP Amount',null,'checkout.log');
        // Mage::log($gotoddpcalculations,null,'checkout.log');
        if(Mage::getConfig()->getModuleConfig($moduleName)->is('active', 'true') && $gotoddpcalculations > 0){
            ...
            // Mage::log('Shipping Country',null,'checkout.log');
            // Mage::log($countrycode,null,'checkout.log');
            ...
            $handling_fee = 0;
            $total_perc = $taxrate->getGstPerc()+$taxrate->getTaxDefaultPerc();
            ...
            // Mage::log('Re-calculated DDP Amount',null,'checkout.log');
            // Mage::log($Total_Custom_Duties_and_Taxes1,null,'checkout.log');
            // Mage::log('Base Re-calculated DDP Amount',null,'checkout.log');
            // Mage::log($Base_Total_Custom_Duties_and_Taxes1,null,'checkout.log');
        }
        $subtot = 0;
        $basesubtot = 0;
        foreach ($_order->getAllItems() as $item) {
            if($Total_Custom_Duties_and_Taxes1 > 0){
            	$store_product_ddp_values = array();
            	$store_product_ddp_values['product_id'] = $item->getProductId();
            	$store_product_ddp_values['price_inr'] = $item->getRowTotal()*$total_perc;
            	$store_product_ddp_values['cur_pric'] = $item->getBaseRowTotal()*$total_perc;
            	$store_product_ddp_values['cur_type'] = $current_currency;
            	$store_product_ddp_values['order_id'] = Mage::getSingleton('checkout/session')->getLastRealOrderId();
            	$store_product_ddp_values['autoincrementid'] = Mage::getSingleton('checkout/session')->getLastOrderId();
            	// Mage::log('Store DDP Product Values',null,'checkout.log');
             //    Mage::log($store_product_ddp_values,null,'checkout.log');
            	$storeddpvalues = Mage::getModel('taxrate/flatrates')->setData($store_product_ddp_values)->setId($store_product_ddp_values['id'])->save();
		        try {
		            if ($storeddpvalues->getCreatedAt == NULL) {
		                $storeddpvalues->setCreatedTime(now());
		            }
		            $storeddpvalues->save();
		        } catch (Exception $e) {}

            	// Mage::log('Row Total',null,'checkout.log');
             //    Mage::log($item->getRowTotal(),null,'checkout.log');
             //    Mage::log('Base Row Total',null,'checkout.log');
             //    Mage::log($item->getBaseRowTotal(),null,'checkout.log');
                $row = 0;
                $baserow = 0;
                ...
                // Mage::log('New Row Total',null,'checkout.log');
                // Mage::log($row,null,'checkout.log');
                // Mage::log('New Base Row Total',null,'checkout.log');
                // Mage::log($baserow,null,'checkout.log');
            ...
            if(sizeof($stored_stitchings) > 0) {
                // Mage::log('Stored Stitchings',null,'checkout.log');
                foreach ($options as $key => $value) {
                    if (isset($stored_stitchings[$key]) && $stored_stitchings[$key] > 0) {
                        // Mage::log('Stored Stitchings1',null,'checkout.log');
                        ...
                        // Mage::log('Stored Stitchings2',null,'checkout.log');
                        if ($is_submited->getMeasurementrememberId() > 0) {
                            // Mage::log('Stored Stitchings3',null,'checkout.log');
                            // Mage::log('Update Stored Measurements',null,'checkout.log');
                            ...
                            $model->save();
                            // Mage::log('Stored Measurements are Updated',null,'checkout.log');
                        }
                    }
                }
            }

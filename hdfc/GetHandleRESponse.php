...
try {
	...
	Mage::log('Response', null, 'res.log');
	Mage::log($strResponseIPAdd, null, 'res1.log');
	Mage::log($ResTrackID, null, 'res2.log');
	...
	if ($strResponseIPAdd != "221.134.101.175" && $strResponseIPAdd != "221.134.101.166" && $strResponseIPAdd != "221.134.101.167" && $strResponseIPAdd != "221.134.101.187" && $strResponseIPAdd != "125.17.18.200" && $strResponseIPAdd != "119.226.198.10") {
		...
		*/
			$REDIRECT = 'REDIRECT=https://www.mysoresareeudyog.com/checkout/onepage/failure/?ResError=--IP MISSMATCH-- Response IP Address is: '.$strResponseIPAdd;
		echo $REDIRECT;
	} else {	
		...
		Mage::log($ResResult,null,'res1.log');
		Mage::log($ResPaymentId,null,'res2.log');
		Mage::log($ResAuth,null,'res3.log');
		Mage::log($ResRef,null,'res4.log');
		Mage::log($ResTranId,null,'res5.log');
		Mage::log($ResTrackID,null,'res6.log');
		Mage::log($ResAmount,null,'res7.log');
		Mage::log($ResErrorText,null,'res8.log');
		Mage::log($ResErrorNo,null,'res9.log');
		...
		if ($ResErrorNo == '') {          
			...
			*/
			...
			if ($ResResult == 'CAPTURED' || $ResResult == 'APPROVED') {
				...
				if ($hashvalue == $Resudf5) {
					...
					/* Hashing Response Successful	*/
						...
						$REDIRECT = 'REDIRECT=https://www.mysoresareeudyog.com/checkout/onepage/success?ResResult=TRANSACTION SUCESSFUL&ME_TX_ID='.$ResTrackID;
					echo $REDIRECT;
				} else {
					/* NOTE - MERCHANT MUST LOG THE RESPONSE RECEIVED IN LOGS AS PER BEST PRACTICE */
					...
					/* Hashing Response NOT Successful */
						$REDIRECT = 'REDIRECT=https://www.mysoresareeudyog.com/checkout/onepage/failure?ResError=HASHING RESPONSE MISMATCH';
					echo $REDIRECT;														
				}
			} else {
				/* PAYMENT GATEWAY RESPONSE IS NOT CAPTURED */
					$REDIRECT = 'REDIRECT=https://www.mysoresareeudyog.com/checkout/onepage/failure?ResError=TRANSACTION FAILURE';
				echo $REDIRECT;														
			}
		} else {
			...
			AND THEN REDIRECT CUSTOMER ON RESULT PAGE*/
				...
				$REDIRECT = 'REDIRECT=https://www.mysoresareeudyog.com/checkout/onepage/failure?ResResult='.$ResResult;
			echo $REDIRECT;
		}
	}	
}
